...
local
   ...
   |   mergeTR (lx as (sX,fX:real) :: xs ) (ly as (sY,fY:real) :: ys) acc =
...
local
   fun mergeTR [] ys acc = List.revAppend (acc, ys)
   |   mergeTR xs [] acc = List.revAppend (acc, xs)
   |   mergeTR (lx as (sX,fX:real,index1) :: xs ) (ly as (sY,fY:real,index2) :: ys) acc =
...
fun timeCalc (p1:int, s1:real, i1) (p2:int, s2:real, i2) = ((Real.fromInt(p2-p1))/(s1-s2))


(* This function compares the second elements of two tuples *)
fun compTuples (x1,y1:real) (x2,y2:real) = if (y1 < y2) then true else false
...
fun loopInt2 j [] myArray n = myArray
    ...
    | loopInt2 j (hd1::hd2::xs) myArray n = 
        if (j <> n) then
            let
                val time = (#3(hd2), (timeCalc hd1 hd2))
                val k = (#2 time > 0.0)
